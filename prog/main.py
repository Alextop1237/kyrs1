...
class EditDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Для таблицы roles добавляем только допустимые значения
        if self.table_name == 'roles':
            self.setup_roles_ui(columns)
            return
    ...
    def setup_roles_ui(self, columns):
        """Специальная настройка UI для таблицы roles"""
        # Получаем допустимые значения для user_role из ограничения CHECK
        constraints = self.db.get_check_constraints('roles')
        allowed_roles = ['admin', 'user', 'manager']  # значения по умолчанию
        
        for constraint in constraints:
            if 'user_role' in constraint[1]:  # ищем ограничение для user_role
                # Парсим условие CHECK (например: "user_role = ANY (ARRAY['admin'::text, 'user'::text, 'manager'::text))")
                import re
                matches = re.findall(r"'(.*?)'", constraint[1])
                if matches:
        ...
        for i, column in enumerate(columns[1:]):
            if column == 'user_role':
                ...
                if self.record and self.record[i+1]:
                    index = combo.findData(self.record[i+1])
                    if index >= 0:
                        combo.setCurrentIndex(index)
            else:
                field = QLineEdit()
...
class TableTab(QWidget):
    ...
    def add_record(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.table_name == 'users':
                if not self.validate_user(data):
                    return
    ...
    def edit_record(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.table_name == 'users':
                if not self.validate_user(new_data, record_id):
                    return
            if self.db.update_record(self.table_name, record_id, new_data):
                self.load_data()
    
    def validate_user(self, data, exclude_id=None):
        username = data[0] if len(data) > 0 else ""
        
        if not username:
            QMessageBox.warning(self, "Ошибка", "Имя пользователя не может быть пустым")
            return False
            
        query = sql.SQL("SELECT id FROM users WHERE username = {}").format(sql.Literal(username))
        if exclude_id:
            query = sql.SQL("{} AND id != {}").format(query, sql.Literal(exclude_id))
            
        existing = self.db.execute_query(query, fetch=True)
        if existing:
